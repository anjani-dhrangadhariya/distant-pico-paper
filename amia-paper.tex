...
Several legacy clinical applications are also supported by rule-based \textit{if-else} systems relying on keyword cues that aid weak labelling. 
The challenge for weak supervision is efficiently combining these several weak labelling sources of varying accuracy. % References
...
The effectiveness of combining several weak labelling sources has been explored by Fries \textit{et al.}~\cite{fries2021ontology} in their Trove system.
...
PICO categories are fuzzier in comparison and much intricate in that they can be subdivided into subclasses.
...
As a result, weakly-supervised PICO entity extraction has not garnered enough attention as much as supervised PICO span extraction.
...
This work demonstrates the feasibility of weakly-supervised PICO entity extraction using two benchmarks.
