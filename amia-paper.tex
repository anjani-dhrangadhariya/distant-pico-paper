...
There have been some fundamental works investigating how to combine multiple labelling sources of variable accuracy by ranking their importance based on their probability of error and efficiently assembling them.
...
The dataset has annotations at two levels: span-level or coarse-grained PICO and entity-level or fine-grained PICO.
...
As EBM-PICO dataset comes pretokenized and we did not do additional preprocessing.
Complementary token information like POS tags and token lemmas were obtained using spaCy.
%
%
%
\begin{table}[h!]
...
%==============================
%
PICO labelled training set development is a token labeling problem where each token in a sequence X is classified or labeled into one of the three PICO classes (Y).
Y is not a manifest variable so our challenge was to devise data-centric techniques to get token labels corresponding to PICO classes and sub-classes to create a labeled dataset.
...
Task) Explain the PICO subclasses here. 
Explain it is a task related to fine-grained PICO detection.
Map the reason to fine-grained annotations from the EBM-PICO corpus.
%
%
%
%==============================
...
We removed non-English and zoonotic source terminologies as well as sources containing fewer than 500 terms remaining with 131 vocabularies.
Preprocessing of UMLS ontologies: Remove stopwords (NLTK, Gensim, scikit learn, spaCy), numbers and punctuation's.
The non-UMLS vocabularies included disease ontology DO, Human Phenotype Ontology (HPO), Ontology of adverse events (OAE), Chemical entities of biological interest (ChEBI),  comparative toxicogenomics database (CTD) Chemical and Disease subclasses.
Our rule-based approach included a combination of hand-crafted dictionaries, regular expressions (ReGeX), reGeX combined with heuristics and knowledge-base distant supervision. 
...
%==============================
%
Task 1) Mapping UMLS semantic types to PICO subclasses UMLS is a compendium of controlled medical vocabularies, concepts in which are mapped to a broader semantic ontology of 128 semantic types.
