...
%==============================
%
Programmatic PICO training set development is a token labeling problem where each token in a sequence X is classified into one of the four PICOS classes (Y).
As ``Intervention'' and ``Comparator'' entities represent interventions in two different roles in clinical studies and semantically the same classes, they are clubbed into a single ``Intervention'' entity class.
Here the Y is not observable, hence the primary challenge was to devise several weak labellers to obtain entity labels.
As Y is not actually observable, a primary technical challenge is estimating Y from multiple, potentially conflicting label sources of unknown quality to construct a probabilistically labeled dataset.
...
It has to be noticed that PICO labels in EBM-PICO are spans composed of multiple fine-grained PICO subclasses.
The participant span can include information about different participant characteristics: disease, disorder, symptoms, age, gender, the sample size in clinical studies, ethnicity, language or geographical location.
The intervention span can include information about pharmaceutical drugs, chemicals, physical therapies, surgery, behavioural or educational therapies, and placebo-based comparators.
Additional intervention information that could be marked is dosage, frequency and mode of administration of intervention.
The outcomes can be subjective, objective, economic outcomes like cost or hard outcomes like mortality.
The spans can also include how these outcomes were measured by having information about the scales, techniques and instruments used to measure them.
Our task was to devise weak labellers corresponding to this fine-grained information, aggregate the labellers, correct the errors in fine-grained PICO EBM-PICO test set and evaluate the approach on both the EBM-PICO and it's corrected version.
%
%
%
%==============================
...
%==============================
%
This labeling function is a weak classifier that uses a labelling sources to label (Y) the unlabeled sequence of input tokens (X).
A labelling function should output +1 for the positive class tokens, -1 for the negative class tokens, and abstain (0) on the text tokens where decision-making is confusing.
We designed four types of labelling functions depending on the types of labelling sources.
The ontology or dictionary labelling functions that takes a class specific dictionary of terms mapped to one of the Y labels.
To account for fuzzy span matching from the ontology concepts, bigram word co-occurrence graphs were used.
A ReGeX labelling function that takes class specific regex patterns for -1, +1 classes and abstains on the rest for that class.
A labelling function combined with heuristics is personalized for each of the PICOS classes and takes a generic regex pattern along with POS tag filters.
These heuristics labelling functions are used to take into account the generic patterns not captured by the structured terminologies.
...
The abbreviations with these classes are then used by ontology labellers.
%
%
%
%==============================
\subsection{Domain insights}\label{lfs}
%==============================
%
The concepts in UMLS vocabularies are organized under a semantic type ontology with 127 semantic types.
Mapping UMLS semantic types to PICO subclasses UMLS is a compendium of controlled medical vocabularies, concepts in which are mapped to a broader semantic ontology of 128 semantic types.
...
Non-UMLS vocabularies that did not provide semantic-type assignments and hence were mapped to a single class label.

The distant supervision dictionaries were created from the structured fields of clinicaltrials.gov. 
The structured field ``Condition or Disease'' was mapped to Participants, ``Intervention/Treatment'' field was mapped to Intervention. The semi-structured ``Primary Outcome Measures'' and ``Secondary Outcome Measures'' fields were mapped to Outcomes class.
