...
Majority voting has higher recall in comparison to precision for obvious reasons. %Anjani: Recall vs. Precision: a debate in PICO extraction.

\paragraph{LF tiers}
Adding non-UMLS LFs increases performance for the intervention target on an average of 4.48\%, but the other classes see a reduced average F1 performance by 0.36\% and 0.64\% for the participants and outcomes targets. 
Adding task-specific LF's increased the average performance by a negligible 0.98\%.
Heuristics improved performance for weakly supervised intervention extraction by about 11.1\%.
%
%
\begin{table}[!ht]
    \centering
