...
The reviewers must collect as many candidate publications as possible, identifying the relevant ones from these and integrating their results via statistical analysis.
...
Weakly-supervised PICO entity recognition has not garnered enough attention as supervised span recognition.
...
We extended these fine-grained classes further to better query the labelling sources and design labelling functions (see Figure~\ref{fig:target_subgroups}).
For example, it's easier to search for ontologies representing adverse events or diseases rather than for the entire participant or outcome spans.
...
We designed $m$ labelling functions $\lambda^{m}$ corresponding to this fine-grained information, aggregated them to get probabilistic token labels, and used the weakly-labelled dataset for downstream PICO recognition tasks.
%
\begin{figure}[ht]
\centering
...
\paragraph{Error rectification}  We also rectified the fine-grained PICO annotations errors in the EBM-PICO test set and evaluated the approach on both the EBM-PICO and its updated version.
...
Though the weakly-supervised PubMedBERT models did not always improve the performance compared to their label model counterparts, they led to the best F1 score for the each target class.
Majority voting has higher recall in comparison to precision for obvious reasons.
...
\begin{table}[!ht]
    ...
    \begin{tabular}{|l|l|l|l|l|l|l|l|l|l|l|}
        ...
        O & UMLS & 0 & 55.59 & 59.86 & 61.25 & 61.25 & 60.85 & 63.49& \textbf{81.20} & 80.53 \\ 
        ~ & +Ontology & 0 & 5.92 & 59.58 & 59.38 & 62.52 & 60.77 & 62.5 & ~ & ~ \\ 
        ~ & +Rules & 0 & 55.08 & 59.36 & 60.88 & 62.86 & 59.15 & 60.39 & ~ & ~ \\ \hline
    \end{tabular}
...
%==============================
%
Our weakly-supervised experiments show promising results for a very complicated PICO recognition task using medical and non-medical ontologies and expert-generated rules.
It is easy to re-purpose the low-cost ontologies but challenging to map these to our targets which is shown by a decreased F1 performance after addition of non-UMLS ontologies.
