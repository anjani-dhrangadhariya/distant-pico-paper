...
Terms in the selected vocabularies were preprocessed by removing stopwords, numbers, punctuations and were lower-cased using Smart lower casing to preserve abbreviations.
...
Vocabularies are structured, standardized data sources that do not capture writing variations from clinical literature and custom-built ReGeX are restricted by either task or even entity type~\cite{ratner2017snorkel,safranchik2020weakly}.
...
We did notice an increasing performance for the first few partitions but did not see the performance drop with a further increase in the partition number in contrast to Trove (refer Figure~\ref{fig:partitions}).
%
\begin{figure} %Anjani: Improve the figure quality
    \centering
