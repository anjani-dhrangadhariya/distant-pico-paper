...
%
\begin{equation}
\bm{\hat{Y}} = \max_{{y \subset \{ 0, 1 \} }} \sum_{i=1}^{m} \mathds{1} (\lambda_{i})
\end{equation}
%
...
GridSearch was used to fine tune the parameters of the label model using the hand-labelled validation set from the EBM-PICO. % Anjani ( TODO: Formula of how the data programming model aggregates the labelling functions in the label matrix and generates probablities)
%
%
%
%==============================
...
% Anjani: Noise-aware loss functions
...
\caption{\label{fig:target_subgroups} Hierarchical representation of PICO target subclasses. The categories marked in \textbf{\textit{bold-italic}} are the same as the fine-grained categories in EBM-PICO corpus.}
\end{figure}
%
%
%
