...
Word count: 3851 %Anjani TODO - Update before submission
...
\begin{table}[!ht]
    ...
    \end{tabular}
    \caption{ Macro-averaged F1 scores for UMLS, UMLS+other and rule-based weak supervision. Underlined values show the best score without manually labelled training data. Note: Fine = EBM-PICO fine-grained annotations, Corr = EBM-PICO fine-grained annotations (EBM-PICO updated)}
    \label{tab:res}
\end{table}
%
...
We noticed an increasing performance for the first few partitions but did not see the performance drop with a further increase in the partition number for the participant target.
For the outcomes target, an increase in the number of partitions leads to an increased performance initially but to a drop with further increase in the partition numbers.
This is in contrast to Trove where an increase in partitions leads to a drop in performance across targets (see Figure~\ref{fig:partitions}).
LM outperforms MV on training performance across the targets and experiments.
%
\begin{figure}
    \centering
...
The errors for PIO categories were rectified, and the updated dataset is made available on Zenodo (DOI: 10.5281/zenodo.7138641)~\footnote{\url{https://doi.org/10.5281/zenodo.7138641
}}.
%
%==============================
