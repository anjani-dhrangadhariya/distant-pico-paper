...
Programmatic labelling is quick and efficiently modified as per the downstream application changes.
Medical ontology compendium like UMLS with its 224 terminologies, other external vocabularies and open access knowledgebases are an excellent resource for building entity labelled training data. 
Legacy clinical applications both in NLP and image analysis are often automated using a combination of rule based approaches (refer to document and image classification methods that are rule based).
Combining multiple labeling sources of variable accuracy is one of challenges.
Weakly-supervised learning has demonstrated its prowess for clinical document classification and relation extraction in NLP, but clinical entity extraction tasks have relied on fully-supervised approaches.


Combining multiple weak labeling sources and clinical entity recognition have been explored by Fries et al. in their paper Trove.
Trove uses Snorkel which is a generative model implemented in python and is domain agnostic methodology to combine multiple labelling sources of variable accuracy.
However, Trove only explores the very defined entities like chemical, disease, disorder and drug classes. 
By definition, PICO information are spans by definition that encompass multiple sub-class entities.
...
Weakly-supervised PICO entity extraction has not garnered enough attention.
As far as our knowledge goes, only two studies exist for weakly-supervised PICO extraction.
One of these approaches only explores weak supervision for intervention extraction using a single labelling source, and the other approach relies on a single source of programmatic labelling and focuses on PICO spans and sentences rather than entities.
...
PICO terms are not standard and even the experts do not agree on the exact span constituting PICO.
...
Consideration 2: Mapping the several ontologies, vocabularies and terminologies to the PICO target sub-classes.
...
Consideration 4: Extending the ontology-based classifiers using expert-generated rules and evaluate performance in comparison to fully-supervised approach.

In this work, we demonstrate the feasibility of weakly-supervised PICO entity extraction using three benchmark test sets.
