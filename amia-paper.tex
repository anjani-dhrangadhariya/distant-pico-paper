...
%==============================
%
A labelling function is a weak classifier $\lambda$ that use domain-specific labelling sources and heuristics to emit a non-zero $\{-1, 0, +1\}$ label for a subset of input tokens.
...
The ontology or dictionary labelling functions for a target class takes a dictionary of terminologies mapped to one of $y \subset \{-1, +1\} $ labels.
To account for fuzzy span matching from the terminologies, relevant bigram word co-occurrences were used.
...
Clinical studies have several abbreviations which were taken into account using heuristics to identify abbreviations from the training text and assigning these to either of the target classes. 
