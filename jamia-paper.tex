...
Depending on the specific SR question, PICO criteria are extended to PICOS (S-Study type or design), PICOC (C-Context),  PICOT (T-timeframe), and PIBOSO (B-Background, S-Study design, O-Other) meaning the static hand-labelled corpora need to undergo costly re-annotation as per the downstream requirements.
...
Aggregation of crowd annotations for spans with fuzzy span boundaries might lead to many boundary errors as well especially when two disparate entities are linked by a \textit{conjunction connector}.
