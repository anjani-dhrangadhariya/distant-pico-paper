...
Finally, we conducted an error analysis to gauge where weak supervision falters compared to full supervision.
...
Many random false positives with terms like ``patients'', ``disease'', <numbers> which could be because of the regex.
FP - numbers following the term ``year''.
Some abb not identified like MetS., TS (testicular cancer), LVH (left ventricular hypertrophy).
Sub group information not identified. 
Outcome symptom identified as participant symptom: eg anxiety measured in patients as a result of treatment, leukopenia, vomitting, nausea, dysrhythmia.
Staging information of disease can be missed for example ``advanced carcinoma'' where advanced is missed out, e.g. 61 ASA physical status I-III patients, primary ureteropelvic junction obstruction.
While some things can be improved upon by incorporating more rules and more dictionaries.
Boundary error brackets and missing extra staging information and numbers following years.
...
Large number of disease abbreviations missed out on.
Anatomy identified as disease e.g. basiler artery, lung.
...
%Anjani: We are at a table limit and if we want to add more tables, we need to remove one of the existing ones.
%
%
%
%==============================
