...
Weakly-supervised PubMedBERT uses the labeled dataset created by the LM to train a downstream model generalizing beyond the LM outputs.
...
The weakly supervised experiments were compared against a fully-supervised PubMedBERT baseline trained using the annotations from EBM-PICO training set.
For all the experiments 80\% of the EBM-PICO dataset was used for training and 20\% for the validation and hyperparameter tuning.
...
Of 12,960 (\textasciitilde1\% of 1,303,169) training tokens evaluated to gauge the errors, 18.30\% of the intervention class tokens, 23.39\% of the participant class tokens and 20.21\% of the outcome class tokens were errors.
...
Therefore, we calculated Cohen's $\kappa_{new}$ between the original EBM-PICO gold set and our re-annotation over 200 documents and compared it to Cohen's $\kappa$ (refer Table\ref{tab:agreement}) provided by the authors of the original corpus.\cite{nye2018corpus}
